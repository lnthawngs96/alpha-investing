...
import type { ModelContext, RegisteredTool, ToolResult, ToolSpec } from '@/types';
import { logAgentAction } from '@/store/agentLog';
...
/** Lấy bề mặt modelContext nếu trình duyệt có (document ưu tiên hơn navigator). */
 ...
 */
export function toToolResult(value: unknown, isError = false): ToolResult {
...
/** Đăng ký một tool với trình duyệt, trả về hàm gỡ. */
function registerOne(mc: ModelContext, tool: RegisteredTool): () => void {
...
/** Spec với args bất kỳ — dùng nội bộ để lưu danh sách hỗn hợp. */
...
export function useWebMCPTools(specs: AnyToolSpec[]): void {
  ...
  useEffect(() => {
    ...
    for (const spec of specsRef.current) {
      ...
      if (typeof window !== 'undefined') {
        const bridge = window.__webmcpTools!;
        bridge[tool.name] = tool;
        cleanups.push(() => {
